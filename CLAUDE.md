...
1. **Every LiteRT-LM call must run off the main thread.** `Engine.initialize()` blocks 4–8 s and will ANR. `LiteRtInferenceEngine` wraps in `Dispatchers.IO` — keep it.
...
42. **Voice I/O is entirely `:androidApp` (no `:shared` seam), behind `ChatSpeaker`/`SpeechDictation`.** Input row order: **mic · speaker · photo · Send**.
    ...
    - **Dictation streams the live transcript into the prompt box (PR #67) — partials are display-only; commands + committed text still key off finalized results.** `Dictation` exposes `partials: Flow<String>` (live, rewrites mid-utterance) **alongside** `results: Flow<String>` (finalized utterances). Android sets `EXTRA_PARTIAL_RESULTS=true` + emits in `onPartialResults`; desktop Vosk emits `recognizer.partialResult`'s `"partial"` field per audio frame (deduped). `ChatScreen` keeps a `committedInput` anchor (stable text) and renders `committedInput (+ space) + partial`; the finalized `results` emission (NONE branch) **or a manual keystroke** promotes the partial into `committedInput`. **`VoiceCommand.match` runs ONLY on `results`** (whole utterances) — never on a partial — so send/cancel/etc. fire exactly as before; the transient command partial flashing in the box is discarded on finalize (`input = committedInput`). Echo suppression (below) drops partials during TTS playback too. **Don't act on a partial** (it's not yet a stable utterance) and **don't send `input`** for a voice SEND — send `committedInput`.
...
48. **Desktop read-aloud (PR #66) shells out per-OS (`say`/`spd-say --wait`/PowerShell `SpeechSynthesizer`) — and stopping it is OS-specific.** `DesktopTtsSpeaker.stop()` calls `process.destroy()`, which silences macOS `say` + the Windows synthesizer (they play in-process) but **NOT Linux**: `spd-say` is only a *client* that hands text to the speech-dispatcher daemon, which keeps playing after the client dies. So `stop()` **also issues `spd-say --cancel`** ("Cancel all messages") on Linux — that's what makes the speaker-mute toggle actually silence in-flight speech there (the shared `ChatViewModel.setTtsEnabled(false)` → `speaker.stop()` path was already correct; the desktop actual was the gap). **Desktop voice IS configurable** (unlike Android): engine (Linux output module, `spd-say -o`) / voice (`-y` / `say -v` / `SelectVoice`) / rate live on the **concrete `DesktopTtsPreferences`** (`tts_prefs.json`), deliberately OFF the shared `TtsPreferences` interface (same split as desktop UI-zoom, #45). `DesktopTtsVoices` enumerates options (`spd-say -O`/`-L`, `say -v ?`, PowerShell `GetInstalledVoices`); the Settings picker is a desktop-only `expect/actual DesktopVoiceSection()` (Android actual = no-op) that reads the concrete prefs + enumerator from Koin. **Real quality win is the bundled Piper neural engine (PR #66), not the OS engine.** `engine == DesktopVoiceConfig.PIPER_ENGINE` ("piper") routes `DesktopTtsSpeaker` to `PiperSpeechSynthesizer` instead of the OS path. Piper is fully self-contained + offline: `PiperBinaryStore` downloads the prebuilt `rhasspy/piper` executable (pinned `PiperRelease.TAG`, per-OS/arch asset table w/ verified sha256, extract tar.gz via system `tar` / Windows zip via `ZipInputStream`, `$ORIGIN`-rpath bundles its own onnxruntime+espeak-ng-data) and `PiperVoiceStore` downloads the ONNX voice + `.onnx.json` (HF `rhasspy/piper-voices`, `en_US-lessac-medium` ≈ 63 MB) — both via the same `DesktopModelDownloader`/`DesktopModelInventory`/app-data layout as the LLM/Vosk models (#download patterns). Synth is `piper --model … --config … --output-raw` (raw S16LE mono @ model sample rate) streamed to a **`javax.sound.sampled.SourceDataLine`** — **no `aplay`/system audio tool, no Python**. So Piper **stops instantly** (in-JVM line close + process kill; the daemon-cancel problem is Linux-`spd-say`-only). Rate maps to `--length_scale` (smaller = faster). The Settings picker offers Piper only when `PiperRelease.assetForHost() != null`; selecting it triggers `prepare()` (pre-download w/ progress in `PiperState`). OS speech-dispatcher modules (espeak-ng default is robotic; RHVoice/pico installed modules appear automatically) remain available as the non-neural fallback.

### Desktop appearance & window (CMP, PR #61)

45. **Desktop UI "zoom" scales `LocalDensity.density`, NOT `fontScale` — and they're different levers.** `AppThemeScaffold` (shared `:ui`) takes `densityScale: Float = 1f` (default ⇒ Android untouched) and applies `Density(base.density * densityScale, base.fontScale * fontScale)`. Density scales `dp`+`sp` (whole UI: icons, padding, forms, text); `fontScale` is the text-only Settings slider. **For "everything too small" use density, never fontScale.** Desktop `Window(onKeyEvent=…)` drives it via **Ctrl/Cmd `+`/`-`/`0`** (modifier-gated so it never eats typed chars); the level is persisted desktop-only on `DesktopThemePreferences.uiZoom*` (`theme_prefs.json`, `UiZoom` bounds 0.8–2.0) — kept OFF the shared `ThemePreferences` interface (mobile has no zoom). `Main.kt` reads the **concrete** `DesktopThemePreferences` from Koin (the zoom methods aren't on the interface).
46. **Desktop theme is monochrome, not the Android green; `ThemeMode.System`/Auto is unreliable on Linux.** Desktop `Theme.kt` defines `LightMonochromeColorScheme` (white surfaces, black trim) + `DarkMonochromeColorScheme` (near-black `#121212`, white trim), with `surfaceTint = Color.Transparent` to kill M3 tonal-elevation overlays (outline gives separation); `error` stays red. Android keeps Material You / the green fallback (desktop-only divergence). **`isSystemInDarkTheme()` on desktop is backed by Skiko — works on macOS/Windows, but on Linux it commonly reports light with no live update (JetBrains CMP-6028).** No Linux detector wired (user's call); use the manual Light/Dark selector there. Status dots (memory/connection green-yellow-red in `:ui`) are semantic, not theming — leave them.
