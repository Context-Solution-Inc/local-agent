...
- `docs/OLLAMA_REMOTE_LLM_PLAN.md` — remote Ollama server for the chat LLM (PR #56, see #44)
...
- **Remote LLM (both platforms, optional, PR #56):** the user can point the *chat LLM* at a LAN **Ollama** server (Settings → Ollama server). `RoutingInferenceEngine` (commonMain) wraps the local engine and routes to `OllamaInferenceEngine` (pure-Ktor, OpenAI-compatible `/v1/chat/completions`, text + image) when configured + reachable, else falls back to local. The classifier/embedder/search/memory always stay on-device. See `docs/OLLAMA_REMOTE_LLM_PLAN.md` and #44.
...
- **Status:** M0–M6 complete (`PHASE1_PLAN.md` §5). M7 (closed beta → Play Store) not started. **Desktop port (Linux/macOS/Windows, Compose Desktop): Phases 0–9 complete, MERGED to `main` via PR #53 (v0.1.0, merge `8963a06`, 2026-06-02); on-device-validated on Linux + Pixel 7. `v0.1.0` git tag still pending.** — see `docs/DESKTOP_PORT_PLAN.md`. **PR #55 reworked the desktop LLM runtime: JNI binding → `llama-server` subprocess (vision + GPU; merged `d4dfa91`); see `docs/DESKTOP_LLAMA_SERVER_PLAN.md`.** **PR #56 (open, draft, branch `feature/ollama-remote-llm`) adds an optional remote-Ollama backend for the chat LLM on both platforms — do NOT merge until on-device validation; see `docs/OLLAMA_REMOTE_LLM_PLAN.md` and #44.**
...
44. **The remote-Ollama path (PR #56) lives entirely behind the `InferenceEngine` seam — never special-case it above the seam.** `RoutingInferenceEngine` is `single<InferenceEngine>` on both platforms; it decides backend **once per `loadModel`** (configured+reachable→Ollama, configured+unreachable→local, unconfigured→local) and tags the `RoutedHandle`. Because it sits **below** the Android `InferenceSessionManager` and desktop `WarmModel`, the agent loop, session lifecycle, foreground service, and watchdogs are unchanged. **Two things must drop the resident handle so the next turn re-decides:** (a) a Settings change — `MobileAgentApplication`/`Main` observe `OllamaPreferences.configFlow()`; (b) a server offline/online transition — `OllamaConnectionMonitor`. `OllamaInferenceEngine` calls the monitor on a **connection** failure (NOT an HTTP-status error — server up ⇒ leave as a plain error) and on a successful connect; the monitor emits `reloadRequests` that the same owners observe (`forceUnload(Manual)` / `WarmModel.invalidate()`), and runs a ~15 s background health-watch so it reconnects automatically once the server returns. **`Accelerator.REMOTE`** is reported by the Ollama handle (delegated up through `RoutedHandle`); the chat `SessionBanner` renders nothing for `Loaded(REMOTE)` (the on-device "Loaded on CPU/GPU…" banner is local-only) and `LiteRtInferenceEngine.backendFor` rejects it. **Image turns** use the configured vision model, else the chat model (no cross-backend switch mid-handle). The streaming client must be **un-timed** — the `HttpEngineFactory`'s 10 s request timeout aborts a long stream, so `OllamaInferenceEngine.loadModel` rebuilds it with `requestTimeoutMillis = null`. Android needs cleartext (`network_security_config.xml`) for `http://` LAN servers.
