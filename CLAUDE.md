...
53. **`MOBILEAGENT_HEADLESS=1` runs the desktop agent as a startup service without opening the main window on launch.** The full background runtime (jobs/clock/tasks/link/warm-LLM/aux engines) is launched on `appScope` **before** the `application{}` window block, so headless only changes *window startup*. The tray probe runs first, then: **(a) tray available** → enter `application{}` with `windowVisible = false` (starts minimized to tray; the tray's Show/Shut-down drive it — same machinery as hide-to-tray); **(b) no tray** (display-less server, or the GNOME/Wayland tray trap, #46) → fully windowless, block the non-daemon main thread on a `CountDownLatch`, with a SIGTERM/Ctrl-C shutdown hook mirroring the GUI `shutdown()` (linkServer.stop/warmModel.unload/appScope.cancel, **no** exitProcess). Nothing before the window touches AWT/Skia (`AppIcon` is `lazy`), so the windowless path is display-less-safe. **The single-instance file lock (`<app-data>/.instance.lock`) is shared across modes** → the service and a manual GUI can't both run (intended: one warm model, one job/task consumer). Same env-gate convention as `DI_CHECK`/`MOBILEAGENT_LLAMA_SERVER_VARIANT`. Service templates (systemd user+system, launchd, Task Scheduler) live in `desktopApp/packaging/`; the build→install→provision→autostart guide is `docs/DESKTOP_PACKAGING.md` "Headless / standalone deployment".
