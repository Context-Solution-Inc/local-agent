 ...
 *
 * Captures 16 kHz mono PCM from the default microphone (`javax.sound.sampled`) and feeds
 * it to a Vosk [Recognizer]; each completed utterance's text is emitted on [results].
 * Continuous by construction (the capture loop runs until [stop]), so there's no
 * Android-style single-shot restart dance. `javax.sound.sampled` is the one cross-platform
 * capture API (Linux/macOS/Windows), and an in-process [TargetDataLine] is reclaimed by the
 * OS on JVM exit — nothing to leak.
 *
 ...
 *
 * After a laptop suspend the capture device can stop delivering audio on resume:
 * `TargetDataLine.read()` returns -1 / a run of 0s, or blocks indefinitely. The original
 * loop did `if (read <= 0) continue` forever — Vosk got no audio, yet [isListening] stayed
 * true (mic looked active). Capture is now split into reopenable per-mic sessions: a session
 * ends [SessionEnd.Stale] on a stale read (see [classifyRead]) or when the [stallWatchdog]
 * force-closes a silent line, and the outer loop reopens the mic (keeping the
 * [Model]/[Recognizer] alive) with bounded backoff. Once audio is healthy again the reopen
 * reconnects — no app restart needed.
 *
 * **Caveat — the deeper failure is often system-level.** On Linux a suspend can leave the
 * PipeWire/ALSA capture node wedged so that NO client (the JVM, `arecord`, `pw-record`) can
 * read the mic; only restarting audio fixes it (`systemctl --user restart wireplumber
 * pipewire pipewire-pulse`). The app can't un-wedge that, so when reopening can't recover
 * audio after [MAX_BARREN_SESSIONS], dictation gives up and emits a [notices] message so the
...
class VoskDictation(
    ...
    override val notices: Flow<String> = _notices.asSharedFlow()
     ...
     * Each [runCaptureSession] opens one mic line and reads until it ends; a
            ...
            Model(modelPath).use { model ->
                Recognizer(model, SAMPLE_RATE).use { recognizer ->
                    ...
                    while (coroutineContext.isActive) {
                        ...
                        // Stale: the line died (suspend/resume) or the watchdog force-closed
                        // it. A session that read audio is a normal recovery (reset the guard);
                        // one that produced nothing counts toward giving up so a wedged device
                        ...
                        if (consecutiveBarren >= MAX_BARREN_SESSIONS) {
                            logger("microphone produced no audio after $consecutiveBarren attempts — dictation disabled ($CAPTURE_WEDGED_HINT)")
                            _notices.tryEmit(CAPTURE_UNAVAILABLE_NOTICE)
                            break
                        }
    ...
    companion object {
        ...
        private const val CAPTURE_WEDGED_HINT =
        ...
        /** User-facing banner text (English-only launch, #64); the log carries the full hint. */
        internal const val CAPTURE_UNAVAILABLE_NOTICE =
            "Microphone unavailable — no audio from the mic. After waking from sleep, try restarting " +
                "audio, then toggle the mic off and on."
