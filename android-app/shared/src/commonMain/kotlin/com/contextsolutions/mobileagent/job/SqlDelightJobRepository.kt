 ...
 *
 * Genuine-local writes also notify [bus] so [com.contextsolutions.mobileagent.sync.SyncController]
...
class SqlDelightJobRepository(
    ...
    private fun rowToJob(row: com.contextsolutions.mobileagent.db.Jobs): Job = Job(
        id = row.id,
        name = row.name,
        command = row.command,
        prompt = row.prompt,
    ...
    private fun rowToRun(row: com.contextsolutions.mobileagent.db.Job_runs): JobRun = JobRun(
        id = row.id,
        jobId = row.job_id,
